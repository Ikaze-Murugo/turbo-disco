...
class DevelopmentFriendlyInputValidationMiddleware
{
    /**
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (app()->environment('production')) {
            ...
            // In development, only log suspicious patterns but don't block
            if ($this->detectSuspiciousPatterns($request)) {
                $this->logSuspiciousActivity($request);
            }
        }

        // Validate file uploads in all environments
        if ($request->hasFile('evidence_files') || $request->hasFile('images') || $request->hasFile('files')) {
